...
CalmsToolkit is a Go module with CLI binaries under `cmd/`: `media-requests`, `media-streams`, `media-calendar`, `arr-feed`, and `calmstoolkit-setup`. Shared library code is under `internal/`. Research notes are under `docs/research/`, user docs under `docs/user/`, and action examples in `docs/user/EXAMPLES_MEDIA_REQUESTS.md`. The `bin/` directory is generated by builds and is gitignored, while the Makefile drives reproducible compilation.
